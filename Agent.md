# JaguarClaw Agent 启动前置文档

> 最后更新：2026-02-07（MVP 完成版）
> 适用范围：当前工作区 `jaguarclaw`（后端 `src` + 前端 `jaguarclaw-ui`）
...
JaguarClaw 是 OpenClaw 的 Java 学习版复刻，目标是做出可教学、可运行、可扩展的 Agent 系统。
...
1. `docs/plans/2026-02-02-jaguarclaw-design.md`（全局设计）
...
12. `jaguarclaw-ui/src/composables/useChat.ts`（前端事件编排 + SubAgent 面板控制）
...
6. **更多内置技能**：丰富 `workspace/.jaguarclaw/skills/` 技能库
