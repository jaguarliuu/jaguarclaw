...
JaguarClaw 是 OpenClaw 的 Java 版复刻，同时作为 AI Agent 实战课程的教学项目。核心目标是让学员理解 Agent 的完整架构：从 WebSocket 网关、ReAct 执行循环、工具系统、Skills 机制、Memory 记忆系统到 Cron 自动化。
...
```
jaguarclaw/
  src/main/java/com/example/jaguarclaw/
